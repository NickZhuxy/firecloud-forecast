...
8 任务全部完成,每任务一提交在 `feat/59-refine-default`。离线套件 587 passed(+8 deselected integration)。

**真实样本基准**(Task 7,warm 缓存 33s):长三角 2026-06-30、threshold 0.40 → `refined=98 sampled=12 MAE=0.0000 P90=0.0000 max=0.0000` —— 全国共享 cube 精修值与单点真值在真实数据上**逐位一致**(同一 (run,fxx) 解码数据集的不同 bbox 裁剪,理论上就该相等,现已实证)。

**全国实弹首跑**(Task 8,`firecloud --date 2026-06-30 --event sunset`,默认 refine):
- 37,697 格,**7912 候选 ≥0.50**(全国全境比 §2 子框的 1419 大得多——含新疆西藏,跨 f004–f009 时次)→ 护栏取前 4000,`cells_skipped=3912` 日志+metadata 双可见
- `cubes_fetched=123`(hour×tile 组)/ 95 tiles,但 `cube_download_bytes=630MB` = 只有 3 块新下载(f004/f008/f009;f005–f007 磁盘命中且正确不计入)——"每周期一次、与 tile 无关"在生产 metadata 直接可读
- wall 4003s(其中 f004 一块在慢网上就下了 866s,f009 中途连接重置被自愈重试),peak_mem 1.9GB(6 个全球数据集驻留 `_ds_cache`)
- PNG:精修格小黑点 + caption "4,000 cells ray-trace refined";被光追降级出显示阈值的格子留点无色,天然呈现"验证过但压掉"
- 暖缓存重跑成本 ≈ 纯计算(4000 格 ≈ 25–30 分钟);`--no-refine` 秒级出 screen 版

**遗留观察**(不阻塞,供后续):活跃日全国候选可超 max_refine_cells=4000,被跳过者维持 screen 值(偏乐观)。当前语义已透明(log/metadata/PNG 无点);若要更强可后续引入"跳过格降权"或分优先级两轮精修。peak_mem 1.9GB 提示按周期释放 `_ds_cache` 的优化空间(§5 曾建议)。

**追加(同日,commit 5311153)——按时次释放 `_ds_cache`**:refine hour 主序处理 + `GFSSource.release_cube`。全国全量暖缓存重跑:**peak_mem 1889 → 518 MB(−73%)**,**wall 4003s → 210s**(首跑的 67 分钟里约 60 分钟是慢网下载;暖缓存纯计算全国 4000 格光追只要 3.5 分钟,~0.05s/格),`cube_download_bytes=0`(零网络)。真实基准 MAE 仍 0.0000。遗留观察里的内存项已消化;跳过格降权与 0.30–0.50 安全带待设计流程。
